...
def compute_w_loader(file_path, output_path, model, batch_size=8, verbose=0,
                     print_every=20, target_patch_size=-1):
    """
    args:
        ...
        target_patch_size: the desired size of patches for optional scaling before feature embedding
    """
    dataset = Whole_Slide_Bag(file_path=file_path, target_patch_size=target_patch_size)
    x, y = dataset[0]
...
if __name__ == '__main__':
    ...
    elif args.model_name == 'facebookresearch/deit_base_patch16_224':
        model = torch.hub.load('rwightman/gen-efficientnet-pytorch', 'efficientnet_b0', pretrained=True)
    else:
